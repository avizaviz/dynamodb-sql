...
function DynamoSQL($config) {
    ...
    if($config && $config.region === 'localhost' || process.env.x_invokeDynamoLocally){
        this.client = new AWS.DynamoDB({maxRetries: 30,region:process.env.x_region, endpoint:'http://localhost:8000'})
    }
    else if ($config instanceof AWS.DynamoDB) {
        this.client = $config
    } else {
        if ($config){
            if ($config.config){
                this.client = new AWS.DynamoDB($config.config)
            }else {
            ...
            var region = process.env.x_region?process.env.x_region:"us-west-1"
            this.client = new AWS.DynamoDB({maxRetries: 30,region:region})
        }
                ...
                Object.keys($parsed.o.WHERE).map(function (k) {
                    switch ($parsed.o.WHERE[k].op) {
                        ...
                        case '><':
                            ...
                            break
                        case '<>':
                            // $db.where(k).defferent($parsed.o.WHERE[k].value);//attribute_not_exists not_null
                            console.log('<>');
                            break;
                        case 'not_null':
                            $db.where(k).not_null($parsed.o.WHERE[k].value);//attribute_not_exists not_null
                            break;
                    }
                })
            }
...
DynamoSQL.prototype.recScan = function ($db,page, data, cb) {
    console.log(`scaning page:${page}\nlimit:${$db.limit_value}`)
    const LIMIT = $db.limit_value;
    var ExpressionAttributeNames = $db.ExpressionAttributeNames
    var ExpressionAttributeValues = $db.ExpressionAttributeValues
    var FilterExpression = $db.FilterExpression
    $db.scan((err, moreData) => {
        console.log("results count:",moreData.length)
        if($db.LastEvaluatedKey){//Scan limit nee to call for next page
            if(LIMIT && moreData.length <= LIMIT){
                cb(err, data.concat(moreData));
            } else {
                $db.ExclusiveStartKey = $db.LastEvaluatedKey;
                $db.ExpressionAttributeNames = ExpressionAttributeNames;
                $db.ExpressionAttributeValues = ExpressionAttributeValues;
                $db.FilterExpression = FilterExpression;
                //Call scan again ask for next page
                this.recScan($db,page+1, data.concat(moreData), cb);
            }
        } else{
            cb(err, data.concat(moreData));
        }
